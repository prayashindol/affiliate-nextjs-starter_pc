...
function fixStructuralIssues($) {
  ...
  // Remove data attributes that are Elementor-specific
  ...
  // Clean up Elementor and WP-related classes
  ...
  $('strong').each(function() {
    ...
    if (!$parent.is('p, li, td, th') && $strong.text().trim().endsWith(':')) {
      ...
      let $next = $wrapper.next();
      while ($next.length && !$next.is('h1, h2, h3, h4, h5, h6') && !($next.is('strong') && $next.text().trim().endsWith(':'))) {
        const $current = $next;
        $next = $next.next();
        $wrapper.append($current);
