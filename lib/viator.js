...
export async function fetchViatorTours({ city, count = 9, revalidateSeconds = 60 * 60 * 12 }) {
  ...
  try {
    ...
    if (process.env.VIATOR_DEBUG === 'true') {
      console.log(`[Viator API] API key present: ${apiKey.length} characters`)
    }
    ...
    if (!metadata.apiError) {
      ...
      const mockMeta = {
        ...
        rawMeta: {
          ...metadata.rawMeta,
          usedMockData: true,
          productsCount: mockTours.length,
          totalTiming: Date.now() - functionStartTime,
        }
      }
      console.log(`[Viator API] Mock data fallback completed in ${mockMeta.rawMeta.totalTiming}ms`)
      
      return { products: mockTours, destinationId: destinationCode, ...mockMeta }
    }
    
    // Set error metadata before re-throwing
    const errorMeta = {
      ...metadata,
      apiStatus: (!metadata.apiStatus || metadata.apiStatus === 'unknown') ? 'error' : metadata.apiStatus,
      apiError: metadata.apiError || error.message,
      rawMeta: {
        ...metadata.rawMeta,
        totalTiming: Date.now() - functionStartTime,
      }
    ...
    // Optionally log or handle errorMeta here if needed
