...
function shapeProduct(raw, destinationId) {
  ...
  const thumbnail = bestImage(raw.images);
  ...
  const shortDescription =
    baseDesc.length > 220
      ? baseDesc.slice(0, 217) + '… <span class="read-more">Read more</span>'
      : baseDesc;

  // Price (check several places across v2/v1 payloads)
  const price =
    raw.price?.formattedValue ||
    raw.price?.fromPrice?.formattedValue ||
    raw.pricingSummary?.lowestPrice?.formattedAmount ||
    raw.fromPrice?.formattedAmount ||
    null;
  ...
  const reviewCount =
    typeof raw.reviewsCount === 'number'
      ? raw.reviewsCount
      : typeof raw.numberOfReviews === 'number'
      ? raw.numberOfReviews
      : 0;

  const rating =
    typeof raw.ratingAverage === 'number'
      ? raw.ratingAverage
  ...
  // Link: safest resolver is /tours/{productCode}
  const link = productCode ? `https://www.viator.com/tours/${encodeURIComponent(productCode)}` : null;
