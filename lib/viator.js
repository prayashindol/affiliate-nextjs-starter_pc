...
function shapeProduct(raw) {
  ...
  // ✅ Image – pick largest available variant
  let thumbnail = null;
  if (Array.isArray(raw.images) && raw.images.length > 0) {
    const variants = raw.images[0].variants || [];
    if (variants.length > 0) {
      thumbnail = variants[variants.length - 1].url; // pick highest-res
    } else {
      thumbnail = raw.images[0].url || null;
    }
  }

  // ✅ Price (from pricing.summary.fromPrice)
  const price = raw.pricing?.summary?.fromPrice
    ? `$${raw.pricing.summary.fromPrice}`
    : null;

  // ✅ Reviews
  const totalReviews = raw.reviews?.totalReviews || 0;
  const rating = raw.reviews?.combinedAverageRating || null;

  // ✅ Duration
  let durationText = null;
  if (raw.duration?.fixedDurationInMinutes) {
    const mins = raw.duration.fixedDurationInMinutes;
    const hours = Math.floor(mins / 60);
    const remMins = mins % 60;
    durationText = hours > 0
      ? `${hours} hr${hours > 1 ? 's' : ''}${remMins ? ` ${remMins} min` : ''}`
      : `${remMins} min`;
  }

  // ✅ Short description w/ inline "Read more"
  let desc = raw.shortDescription || raw.description || '';
  if (desc.length > 250) {
    desc = desc.slice(0, 250) + '… ';
    desc += `<span class="read-more">Read more</span>`;
  }

  // ✅ Link (use productUrl directly)
  const link = raw.productUrl || null;

  return {
    productCode: raw.productCode || raw.code || raw.id || 'UNKNOWN',
