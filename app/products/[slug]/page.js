...
import { sanityClient } from "@/lib/sanity"; // Adjust path if needed
import { StarIcon } from "@heroicons/react/20/solid";
import { useState } from "react";
// ...import all other needed icons and HeadlessUI components...

// Helper to fetch product by slug
async function getProductBySlug(slug) {
  // Adjust fields as per your Sanity schema!
...
export default async function ProductPage({ params }) {
  ...
  // Only render this template for dropship products
  if (product.productType !== "Dropship") return notFound();
  ...
  // For client-side interactivity, you can use a Client Component for the options part

  return (
    <div className="bg-white">
      {/* ...You can move the nav/header/footer to layout if you want... */}
      ...
      <main className="pt-10 sm:pt-16">
        ...
        <div className="mx-auto max-w-2xl px-4 pt-10 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto_auto_1fr] lg:gap-x-8 lg:px-8 lg:pt-16">
          ...
          {/* Options (move to a client component for interactivity) */}
            ...
            <div className="mt-10">
              ...
              <ul className="mt-4 list-disc space-y-2 pl-4 text-sm">
                {highlights.map((highlight, i) => (
                  <li key={i} className="text-gray-400"><span className="text-gray-600">{highlight}</span></li>
                ))}
              </ul>
            </div>
            <section aria-labelledby="shipping-heading" className="mt-10">
              <h2 id="shipping-heading" className="text-sm font-medium text-gray-900">Details</h2>
                ...
                {reviews.featured?.map((review) => (
                  <div key={review.id} className="flex flex-col sm:flex-row">
                    ...
                    <div className="order-1 flex items-center sm:flex-col sm:items-start">
                      {review.avatarSrc && <img alt={review.author} src={review.avatarSrc} className="size-12 rounded-full" />}
                        ...
                        <div className="mt-2 flex items-center">
                          {[0, 1, 2, 3, 4].map((rating) => (
                            <StarIcon
                              key={rating}
                              aria-hidden="true"
                              className={classNames(review.rating > rating ? 'text-gray-900' : 'text-gray-200', 'size-5 shrink-0')}
                            />
                          ))}
                        </div>
                      </div>
