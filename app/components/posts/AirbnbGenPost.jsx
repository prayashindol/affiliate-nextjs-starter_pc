...
// Constants for meaningful content detection
const MIN_MEANINGFUL_CONTENT_LENGTH = 50;

function cleanContentHtml(html, mainImage, permalink) {
  // NOTE: NO Viator-specific fixViatorHtml processing for regular airbnb-gen posts
  ...
  // General cleanup for navigation links by text content
  $('a').each(function () {
...
export default function AirbnbGenPost({ post }) {
  console.log("********* AirbnbGenPost RENDERED *********");
  ...
  // Featured image computation - for regular posts, only show if available (no placeholder)
  ...
  return (
    <article className="max-w-5xl mx-auto py-12 px-4 sm:px-8 lg:px-0">
      ...
      {/* Featured image logic: for non-Viator posts, show image only if available (keep existing behavior) */}
