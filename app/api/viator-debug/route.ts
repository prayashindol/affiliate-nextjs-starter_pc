...
export async function GET(req: Request) {
  ...
  const cats = Array.isArray(post?.categories) ? post.categories.map((c:any)=>c?.slug?.current || c?.slug) : [];
  const viatorByStringCategory = Array.isArray(post?.category) && post.category.includes('airbnb-gen-viator');
  const isViatorByCategories = cats.includes('airbnb-gen-viator');
  const viator = post?._type === 'seoGenPostViator' || isViatorByCategories || viatorByStringCategory;
  ...
  let count = 0;
  if (viator && post?.city) {
    const res = await fetchViatorTours({ city: post.city, count: 6 }).catch((e:any)=>({ apiStatus:'exception', apiError: e?.message || String(e), products: [] }));
    viatorMeta = { apiStatus: res.apiStatus, apiError: res.apiError, destinationId: res.destinationId, tried: res.rawMeta?.tried };
